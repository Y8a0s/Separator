...
<div class="d-flex justify-content-center row">
    <div class="col-lg-8">
        <div class="card">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form wire:submit.prevent="updateProduct" class="form-horizontal">
                <div class="card-body">
                    ...
                    <div class="input-group mb-3" dir="ltr">
                        ...
                        @if (!$errors->isNotEmpty())
                            @if ($images){{-- when new images uploaded --}}
                                <div class="d-flex w-100 mt-3" dir="rtl">
                                    @foreach ($images as $image)
                                        <img class="w-25" src="{{ $image->temporaryUrl() }}">
                                    @endforeach
                                </div>
                            @elseif(is_string($product->images) && !is_null($product->images))
                                {{-- when images exist in DB --}}
                                <div class="d-flex w-100 mt-3" dir="rtl">
                                    @php
                                        $images = json_decode($product->images);
                                    @endphp
                                    @foreach ($images as $image)
                                        <img class="w-25" src="{{ Storage::url($image) }}">
                                    @endforeach
                                </div>
                                @else{{-- when doesn't exist any images (default image) --}}
                                <div class="w-100 mt-3" dir="rtl">
                                    <img class="w-25" src="{{ Storage::url('images/products/default-products-image.jpg') }}">
                                </div>
                            @endif
                        @endif
                    </div>
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ route('admin.products.all') }}" class="btn btn-danger float-left">لغو</a>
                </div>
                <!-- /.card-footer -->
            </form>
        </div>
